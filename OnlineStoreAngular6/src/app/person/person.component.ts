...
export class PersonComponent implements OnInit {
  dataSource;
  displayedColumns: string[] = ['choose', 'personID', 'personName', 'age', 'birthday', 'photo',
    'gender', 'relationshipStatus', 'nationality', 'phoneNumber',
    'religion', 'personAddress', 'coordinates'];
  personList: Person[];
  selectPerson: Person;
  ...
  paginate($event) {
    this.personService.getPersonWithPagination($event.first, $event.rows, this.searchPersonName)
      .then(result => {
  ...
  savePerson() {
    ...
    if (this.isAddPerson == true) {
      ...
      this.personService.editPerson(this.selectPerson.personID, this.selectPerson).then(result => {
        result.birthday =
        this.datePipe.transform(this.selectPerson.birthday, 'yyyy-MM-dd');
